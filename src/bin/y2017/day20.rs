...
fn solve_quadratic(a: i32, b: i32, c: i32) -> (Solution, Solution) {
    if a == 0 {
        if b == 0 {
            if c == 0 {
...
// We need to redefine v(t) to be v(t) = (vi + vf)/ 2 = vi + at/2.
// This is because the standard kinematics equation assumes that the motion
// happens instantaneously, whereas it actually occurs in a continous manner.
// That is, the initial velocity (vi) isn't going to be the velocity
// throughout the motion.
macro_rules! detect_collision {
    ($a:ident, $b:ident, $axis:ident) => {{
        let a = $a.a.$axis - $b.a.$axis;
        let b = 2 * ($a.v.$axis - $b.v.$axis) + a;
        let c = 2 * ($a.pos.$axis - $b.pos.$axis);
...
impl Solution {
    fn with(self, other: Solution) -> Solution {
        match (self, other) {
            (Solution::Any, Solution::Any) => Solution::Any,

            (Solution::Exact(a), Solution::Any) => Solution::Exact(a),
...
pub fn part2(s: &str) -> usize {
    ...
    for (_, pairs) in collisions {
        let collided_before = collided.clone();
        for (a, b) in pairs {
            if !collided_before.contains(&a) && !collided_before.contains(&b) {
                collided.insert(a);
                collided.insert(b);
            }
        }
...
impl fmt::Debug for Particle {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        write!(f, "p={:?}, v={:?}, a={:?}", self.pos, self.v, self.a)
    }
}
...
impl Particle {
    fn parse(s: &str) -> Particle {
        Particle {
            pos: Vector::parse(&s[s.find("p=").unwrap() + 3..s.find(">, v").unwrap()]),
            v: Vector::parse(&s[s.find("v=").unwrap() + 3..s.find(">, a").unwrap()]),
            a: Vector::parse(&s[s.find("a=").unwrap() + 3..s.len() - 1]),
        }
    }
}

#[bench]
fn parse_1(b: &mut ::test::Bencher) {
    b.iter(|| parse(INPUT));
}
