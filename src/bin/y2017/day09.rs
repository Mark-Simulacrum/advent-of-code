...
pub fn eval(s: &str) -> (usize, usize) {
    let mut total = 0;
    let mut garbage = 0;
    let mut in_groups = 0;
    let mut in_garbage = false;
    let mut p = Parser::new(s.as_bytes());
    while let Some(ch) = p.read() {
        if in_garbage {
            match ch {
                b'!' => {
                    p.advance();
