...
use advent_of_code::{Parser, ParserError};

#[derive(Copy, Clone, Debug)]
enum Instruction {
    ToBot { value: u8, to: u8 },
    State { bot: u8, low: Output, high: Output },
}
...
impl Output {
    fn parse(p: &mut Parser) -> Result<Output, ParserError> {
        let c = if let Ok(true) = p.expect(b"bot ") {
            Output::Bot
    ...
    fn put(self, value: u8, bots: &mut BTreeMap<u8, Bot>, outputs: &mut BTreeMap<u8, u8>) {
        match self {
            Output::Bot(n) => bots.get_mut(&n).unwrap().chips.push(value),
...
pub fn part1(s: &str) -> usize {
    ...
    loop {
        ...
        for num in checked {
            ...
            low.put(chips[0], &mut bots, &mut outputs);
            high.put(chips[1], &mut bots, &mut outputs);
        }
    }
