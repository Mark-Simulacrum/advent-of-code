...
fn is_real(name: &str, checksum: &str) -> bool {
    let mut map = VecMap::with_capacity(20);
    for ch in name.chars() {
        *map.get_or_insert(ch, 0) += 1;
    }
    let mut most_common = map.into_iter()
        .filter(|&(k, _)| k != '-')
        .map(|(k, v)| (v, k))
        .collect::<Vec<_>>();
    // compare reverse numbers and then letters normally
    most_common.sort_unstable_by(|a, b| b.0.cmp(&a.0).then(a.1.cmp(&b.1)));
    most_common[0..5]
        .iter()
        .cloned()
        .map(|(_, k)| k)
...
fn parse_room(line: &str) -> (&str, usize, &str) {
    let open_br = line.len() - 7; // two braces + 5 characters
    let last_dash = open_br - 4;
    let name = &line[0..last_dash];
    let id = line[last_dash + 1..open_br].parse::<usize>().unwrap();
    let checksum = &line[open_br + 1..line.len() - 1];
    (name, id, checksum)
}
...
pub fn part1(s: &str) -> usize {
    ...
    for line in s.trim().lines() {
        let (name, id, checksum) = parse_room(line);
...
fn decrypt(name: &str, id: usize) -> String {
    name.chars()
        .map(|ch| {
            if ch == '-' {
                ' '
                ...
                let b = ch as u8 - b'a';
                (((b as usize) + (id % 26)) % 26 + b'a' as usize) as u8 as char
            }
        })
        .collect()
}
...
pub fn part2(s: &str) -> usize {
    for line in s.trim().lines() {
        let (name, id, checksum) = parse_room(line);
        if is_real(name, checksum) {
            if decrypt(name, id).contains("northpole object storage") {
                return id;
            }
        }
    }

    panic!("didn't find room")
}
