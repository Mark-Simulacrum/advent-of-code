...
pub fn part1(s: &str) -> usize {
    decompress(s.as_bytes(), false)
}

pub fn part2(s: &str) -> usize {
    decompress(s.as_bytes(), true)
}
...
impl<'a> Parser<'a> {
    ...
    fn consume_until(&mut self, c: u8) -> usize {
    ...
    fn consume_bytes(&mut self, n: usize) -> Result<&[u8], ParserError> {
    ...
    fn consume(&mut self, x: u8) -> Result<(), ParserError> {
        let next = self.cur();
        if next == Some(x) {
            self.advance();
            Ok(())
        } else {
            return Err(ParserError::Unexpected(x as char, next.map(|c| c as char)));
        }
    }

    fn consume_number(&mut self) -> Result<u64, ParserError> {
        let mut i = 0;
        let mut out = 0;
        ...
        if self.at_end() {
            ...
            return Err(ParserError::Unexpected('#', self.cur().map(|c| c as char)));
        }
        Ok(out)
    }
}
...
#[test]
fn part1_7() {
    assert_eq!(part1("(2x1)AA(1x2)C"), "AACC".len());
}
