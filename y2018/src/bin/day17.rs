...
fn parse_record(s: &str) -> Vec<usize> {
    if s.contains("..") {
        let start = s[..s.find('.').unwrap()].parse::<usize>().unwrap();
        let end = s[s.find('.').unwrap() + 2..].parse::<usize>().unwrap();
...
fn generator(input: &str) -> State {
    ...
    for line in input.trim().lines() {
        ...
        let (x, y) = if y_start > x_start {
            let y = parse_record(&line[y_start + 2..]);
            let x = parse_record(&line[x_start + 2..line.find(',').unwrap()]);
            (x, y)
        } else {
            let x = parse_record(&line[x_start + 2..]);
            let y = parse_record(&line[y_start + 2..line.find(',').unwrap()]);
            (x, y)
        };
        for x in x {
            for &y in &y {
                clay.insert(Position { x: x as isize, y });
            }
        }
    }
...
struct State {
    ...
    clays: HashMap<usize, Vec<isize>>,
    water: HashMap<Position, Water>,
    counts: Vec<usize>,
}

impl fmt::Debug for State {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        let min_x = self.clay.iter().map(|p| p.x).min().unwrap() - 1;
        let max_x = self.clay.iter().map(|p| p.x).max().unwrap() + 1;
        ...
        for y in 0..=self.max_y {
            ...
            for x in min_x..=max_x {
                ...
                if self.clay.contains(&pos) {
                    write!(f, "#")?;
                } else if pos.x == 500 && pos.y == 0 {
                    write!(f, "+")?;
                } else if let Some(&state) = self.water.get(&pos) {
                    let c = match state {
                        Water::Flow => "|",
                        Water::Settled => "~",
                    };
...
impl State {
    fn can_flow_into(&self, pos: Position) -> Flow {
        if pos.y > self.max_y {
            return Flow::Abyss;
        }
    ...
    fn check_progress(&mut self) -> bool {
        if self.counts.iter().all(|c| *c == self.water.len()) {
            return false;
        }
        self.counts.rotate_left(1);
        *self.counts.last_mut().unwrap() = self.water.len();
    ...
    fn insert_water(&mut self) {
        ...
        while let Some(mut at) = to_visit.pop() {
            if !current_visited.insert(at) {
                continue;
            }
            visited += 1;
            self.water.insert(at, Water::Flow);
            while self.can_flow_into(at.down()) == Flow::Yes {
                at = at.down();
                self.water.insert(at, Water::Flow);
            }
            ...
            if self.can_flow_into(at.down()) == Flow::Yes {
            ...
            if self.try_settle(center) {
                self.try_settle(center.up());
                to_visit.push(center.up());
            }
    ...
    fn try_settle(&mut self, pos: Position) -> bool {
        let clays = if let Some(clays) = self.clays.get(&pos.y) {
            clays
        } else {
            return false;
        };
        match clays.binary_search(&pos.x) {
            Ok(_) => unreachable!("found clay overlapping with water"),
            Err(e) => {
                if e == clays.len() || e == 0 {
                    return false;
                }
                let before = clays.get(e - 1);
                let after = clays.get(e);
                match (before, after) {
...
#[solution(part1,
    ...
    example = 57,
    expect = 42429)]
fn part1(mut input: Out) -> usize {
    let mut last_count = [300; 600];
    while last_count.iter().any(|c| *c != input.water.len()) {
        input.tick();
        last_count.rotate_left(1);
        *last_count.last_mut().unwrap() = input.water.len();
    }
    input.count_in_range()
}
