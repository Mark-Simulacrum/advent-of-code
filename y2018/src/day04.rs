...
#[solution(part1,
    ...
    example = 240,
    expect = 14346)]
fn part1(mut input: Vec<Record>) -> usize {
    input.sort_by_key(|r| (r.date, r.hour, r.minute));

    let mut guards: fnv::FnvHashMap<u16, (u16, [u16; 60])> = fnv::FnvHashMap::default();
    let mut current_guard: Option<u16> = None;
    let mut last_sleep = None;
    for record in &input {
        match record.event {
            Event::Asleep => {
                last_sleep = Some(record.minute);
            }
            Event::Wake => {
                let asleep_at = last_sleep.take().unwrap();
                let delta = (record.minute - asleep_at) as u16;
                let e = guards.get_mut(&current_guard.expect("guard")).unwrap();
                e.0 += delta;
                for minute in asleep_at..record.minute {
                    e.1[minute as usize] += 1;
                }
            }
            Event::BeginShift { id } => {
                current_guard = Some(id);
                guards.entry(id).or_insert((0, [0; 60]));
            }
        }
    }

    let (id, (_, minutes)) = guards.iter().max_by_key(|(_, v)| v.0).unwrap();
...
#[solution(part2,
    ...
    example = 4455,
    expect = 5705)]
fn part2(mut input: Vec<Record>) -> usize {
    input.sort_by_key(|r| (r.date, r.hour, r.minute));

    let mut minutes = vec![fnv::FnvHashMap::default(); 60];
    let mut guards: fnv::FnvHashMap<u16, [u16; 60]> = fnv::FnvHashMap::default();
    let mut current_guard: Option<u16> = None;
    let mut last_sleep = None;
    for record in &input {
        match record.event {
            ...
            Event::Wake => {
                let asleep_at = last_sleep.take().unwrap();
                let guard_id = current_guard.expect("guard");
    ...
    let mut max_sleep = 0;
    let mut max_minute = 61; // invalid minute
    let mut max_guard = usize::max_value(); // invalid guard
    for (minute, guards) in minutes.iter().enumerate() {
        for (guard, times) in guards.iter() {
            if *times > max_sleep {
                max_sleep = *times;
                max_minute = minute;
                max_guard = *guard as usize;
            }
        }
    }
